...
def chatuser(sname,rname):
        ...
        def display_messages():
            ...
            for message in messages:
                ...
                if sender == rname:
                   ...
                   messages_text.insert(tk.END, display_text)
            print(a)
            messages_text.after(1000, display_messages)
            messages_text.config(state=DISABLED)
            messages_text.yview(tk.END)
    ...
    sendbtn = PhotoImage(file='Images/send.png')

    # username

    usernameEntry1 = Label(window,font=('',15,'underline'), text=f'{rname} :', bd=0, bg='gray50', fg='white')
    usernameEntry1.place(x=550,y=20)
    # usernameEntry1.insert(0,username)


    messages_text = tk.Text(window,bd=2,bg='gray80',highlightthickness=2,highlightbackground='black',height=36,width=150,cursor='arrow',state=DISABLED)
    messages_text.pack()

    entry = tk.Entry(window,bd=1,font=('',18),width=50)
    entry.place(x=80,y=700)


    send_button = tk.Button(window,bd=0,bg='gray50',fg='black',image=sendbtn,activebackground='gray50',cursor='hand2', command=send_message)
    send_button.place(x=735,y=695)
    ...
    back_btn = tk.Button(window, text="back",bd=0,cursor='hand2',command=back)
...
text=Label(win,text='⮞',font=('',500),bg='white').pack()
