...
def chatter(username):
    ...
    def showuser():
        if e1_str.get() in my_list:
            search_window.destroy()
            from chatting import chatuser
            chatuser(username,e1_str.get())
    ...
    searchbtn = Button(search_window, width=20, text='Message', command=showuser)
    searchbtn.place(x=610+110, y=600)
